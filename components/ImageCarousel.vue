<template>
    <div class='w-5/6'>
        <div class='flex w-full'>
            <div class='w-full'>
                <nuxt-img :src="mainImg" quality='80' lazy class='w-full h-72 object-cover rounded-md shadow-light_2xl dark:shadow-2xl mb-4' :class="altImgs.length ? 'h-72' : 'h-96'" />
                <div v-if='altImgs.length' class='grid gap-1' :class="`grid-cols-${altImgs.length}`">
                    <div v-for='img in altImgs' :key='img' >
                        <nuxt-img :src='img' quality='60' lazy class='w-full h-32 object-cover rounded-sm hover:opacity-50 hover:cursor-pointer' @click="mainImg = img"/>
                    </div>
                </div>
            </div>
